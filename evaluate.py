...
def evaluate_one(result_root, model_name, data_split, example):
  ...
  if 'plus_fuse_net' in model_name:
    pred_file = tf.gfile.Glob(example_dir + '/output_image_*')[0]
  else:
    pred_file = tf.gfile.Glob(example_dir + '/syn_image_*')[0]
...
def main(_):
  ...
  for i in range(len(all_data)):
    ssim_sum = ssim_sum + all_data[i][0][0]
    psnr_sum = psnr_sum + all_data[i][1][0]
  ssim_avg = ssim_sum / len(all_data)
  psnr_avg = psnr_sum / len(all_data)
