...
# flags.DEFINE_integer('image_height', 464, 'Image height in pixels.')
# flags.DEFINE_integer('image_width', 864, 'Image width in pixels.')
# flags.DEFINE_integer('image_height', 320, 'Image height in pixels.')
# flags.DEFINE_integer('image_width', 640, 'Image width in pixels.')
# flags.DEFINE_integer('image_height', 400, 'Image height in pixels.')
# flags.DEFINE_integer('image_width', 800, 'Image width in pixels.')
flags.DEFINE_integer('image_height', 384, 'Image height in pixels.')
flags.DEFINE_integer('image_width', 768, 'Image width in pixels.')
# flags.DEFINE_integer('image_height', 352, 'Image height in pixels.')
# flags.DEFINE_integer('image_width', 704, 'Image width in pixels.')
# flags.DEFINE_integer('image_height', 448, 'Image height in pixels.')
# flags.DEFINE_integer('image_width', 848, 'Image width in pixels.')

# flags.DEFINE_integer('sequence_length', 4, 'Sequence length for each example.')
flags.DEFINE_integer('sequence_length', 2, 'Sequence length for each example.')
# flags.DEFINE_integer('min_stride', 3, 'Minimum stride for sequence.')
flags.DEFINE_integer('min_stride', 23, 'Minimum stride for sequence.')
# flags.DEFINE_integer('max_stride', 10, 'Maximum stride for sequence.')
flags.DEFINE_integer('max_stride', 23, 'Maximum stride for sequence.')
...
def create_from_flags(cameras_glob='train/????????????????.txt',
  ...
  # tf >= 1.4; list_files(file_pattern): A dataset of all files matching a pattern
  files = tf.data.Dataset.list_files(cameras_glob, False)
  lines = files.map(datasets.read_file_lines, num_parallel_calls=parallelism)  # maps to tf.contrib.data.get_single_ele
  # Returns: The lines of the file, as a 1-dimensional string tensor.
  sequences = lines.map(
      datasets.parse_camera_lines, num_parallel_calls=parallelism)
  # Returns: ViewSequence(youtube_id, timestamps, intrinsics, poses, images)
  ...
  return Loader(
      ...
      parallel_image_reads=50,
      # prefetch=1)
      prefetch=1, files=files)  # debug Mayza
